...
const DEFAULT_EXTRACT_SYSTEM_PROMPT = [
    ...
    '[5] Link plan: for each relation-worthy update, state link targets (type/title/relation) or justify no-link.',
    '[6] Planned tool calls: list call names in execution order (done must be last).',
    '</thought>',
    'If <thought> misses any of the 6 sections above, treat your own response as invalid and regenerate fully.',
    ...
    'If you decide not to add links in this batch, explain why in <thought> with explicit evidence.',
...
function buildPlainTextToolProtocolMessage(tools = [], { requiredFunctionName = '' } = {}) {
    ...
    return [
        ...
        'You may optionally output one short <thought>...</thought> before JSON calls.',
        'After optional thought, output a parseable JSON call payload.',
        'Do not add extra prose after the JSON payload.',
